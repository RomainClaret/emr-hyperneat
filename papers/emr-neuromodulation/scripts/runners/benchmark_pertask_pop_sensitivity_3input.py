#!/usr/bin/env python3
"""E-S24: Per-Task Pop Sensitivity at 3-Input.

Tests whether per-task activation maintains its advantage at smaller
populations at 3-input (where uniform tanh drops to 6.7% at Pop=100).

At 2-input, per-task achieves 100% across Pop=100-1000 (150 runs).
At 3-input, per-task is only validated at Pop=750 (30/30, 100%).

Conditions: per-task activation (sin for Parity-3, tanh for rest)
Pop = {100, 300, 1500} × 30 seeds = 90 runs
(Pop=750 already tested in E-S9 as 100%)

Architecture: MLP + (mu+lambda)-ES, 5 tasks (Parity-3, AND-3, OR-3, NAND-3, NOR-3)
N_IN=3, N_HIDDEN=20, 200 gens.

Results saved to: papers/emr-neuromodulation/results/pertask_pop_sensitivity_3input/

Usage:
    python papers/emr-neuromodulation/scripts/runners/benchmark_pertask_pop_sensitivity_3input.py
    python papers/emr-neuromodulation/scripts/runners/benchmark_pertask_pop_sensitivity_3input.py \
        --seeds 3 --pop-sizes 100 300  # smoke test
    python papers/emr-neuromodulation/scripts/runners/benchmark_pertask_pop_sensitivity_3input.py --summary
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / 'results' / 'pertask_pop_sensitivity_3input'

N_IN = 3
N_HIDDEN = 20  # Matching E-S9
N_OUT = 1
...
DEFAULT_POP_SIZES = [100, 300, 1500]
...
def eval_single_multitask(params: Dict) -> Tuple[float, Dict[str, float]]:
    ...
    for task in TASK_NAMES:
        ...
        forward_fn = get_forward_fn(act_name)
...
def main():
    ...
    if args.summary:
        print("\n=== E-S24 Per-Task Pop Sensitivity 3-Input Summary ===\n")
    ...
    for pop_size in pop_sizes:
        ...
        for seed in range(args.seeds):
            ...
            fname = RESULTS_DIR / f'pertask_pop{pop_size}_seed{seed}.json'
