#!/usr/bin/env python3
"""E-S9: 5-Task at 3-Input Arity.

Extends the 5-task neuromodulation result (2-input, 100%) to 3-input tasks
to test whether the full 5-task neuromodulation mechanism scales with arity.
...
Tasks (3-input, 8 patterns each):
  ...
  - NAND-3: NOT(AND-3), output 0 iff all 3 inputs = 1
  - NOR-3: NOT(OR-3), output 0 iff any input = 1

Conditions (4 total, 120 runs):
  1. 1layer_5task_uniform_tanh, baseline (Parity-3 should fail)
  2. 1layer_5task_pertask, sin for Parity-3, tanh for all others
  3. 1layer_5task_uniform_sin, oscillatory control
  4. 2layer_5task_uniform_tanh, depth control

Architecture: MLP + (mu+lambda)-ES (consistent with E22/E25 higher-arity)
N=30 seeds, 200 generations, Pop=750, N_IN=3, N_HIDDEN=20.

Results saved to: papers/emr-neuromodulation/results/5task_3input/

Usage:
    python papers/emr-neuromodulation/scripts/runners/benchmark_5task_3input.py
    python papers/emr-neuromodulation/scripts/runners/benchmark_5task_3input.py \
        --seeds 3 --conditions 1layer_5task_uniform_tanh 1layer_5task_pertask  # smoke test
    python papers/emr-neuromodulation/scripts/runners/benchmark_5task_3input.py --summary
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / 'results' / '5task_3input'

N_IN = 3
N_HIDDEN = 20  # Matching existing 5-task experiments scaling
N_OUT = 1
...
ALL_CONDITIONS = [
    '1layer_5task_uniform_tanh',
    '1layer_5task_pertask',
    '1layer_5task_uniform_sin',
    '2layer_5task_uniform_tanh',
]
...
def main():
    parser = argparse.ArgumentParser(
        description='E-S9: 5-task at 3-input arity neuromodulation')
    ...
    if args.summary:
        print("\n=== E-S9 5-Task 3-Input Results Summary ===\n")
