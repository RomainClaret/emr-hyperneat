#!/usr/bin/env python3
"""E-S10: Population Scaling at 3-Input (Uniform Tanh).

Tests whether search budget pushes tanh success higher at the 3-input
transition zone (where tanh=20% at Pop=750).
...
Conditions: uniform tanh ONLY (varying population size)
Pop = {100, 300, 750, 1500} x 30 seeds = 120 runs

Architecture: MLP + (mu+lambda)-ES (consistent with E22/E33)
N_IN=3, N_HIDDEN=15, 200 gens.

Results saved to: papers/emr-neuromodulation/results/pop_sensitivity_3input/

Usage:
    python papers/emr-neuromodulation/scripts/runners/benchmark_pop_sensitivity_3input.py
    python papers/emr-neuromodulation/scripts/runners/benchmark_pop_sensitivity_3input.py \
        --seeds 3 --pop-sizes 100 750  # smoke test
    python papers/emr-neuromodulation/scripts/runners/benchmark_pop_sensitivity_3input.py --summary
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / 'results' / 'pop_sensitivity_3input'
...
def eval_single_multitask(params: Dict, n_layers: int = 1) -> Tuple[float, Dict[str, float]]:
    """Evaluate with uniform tanh activation on all tasks."""
    per_task = {}
    for task in TASK_NAMES:
        nt = NT_VECTORS[task]
        targets = TARGETS[task]

        forward_fn = get_forward_fn(n_layers, 'tanh')
...
def main():
    parser = argparse.ArgumentParser(
        description='E-S10: Population scaling at 3-input (uniform tanh)')
    ...
    if args.summary:
        print("\n=== E-S10 Pop Sensitivity 3-Input (Tanh) Summary ===\n")
