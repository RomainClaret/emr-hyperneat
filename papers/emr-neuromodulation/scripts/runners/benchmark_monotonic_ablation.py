#!/usr/bin/env python3
"""E-S21: Monotonic Activation Ablation, H2 Strengthening.

Tests H2 claim that "all monotonic activations fail" by testing 5 monotonic
activations beyond just tanh. The mathematical argument (monotonic function
maps sum=1→HIGH, so sum=2→HIGH) is activation-agnostic, but empirical
confirmation across the monotonic class is needed.

Conditions (5 × 30 seeds = 150 runs):
  1. uniform_tanh, replication control (known 76.7% at 2-input)
  2. uniform_sigmoid, bounded monotonic, different saturation profile
  3. uniform_relu, unbounded monotonic, zero for negatives
  4. uniform_leaky_relu, unbounded monotonic, small negative slope
  5. uniform_identity, linear, no nonlinearity at all

Architecture: Direct-encoded MLP + (mu+lambda)-ES, consistent with E-S6/E-S7.
5 tasks (XOR, AND, OR, NAND, NOR), 2-input, N_HIDDEN=10, Pop=750, 100 gen.
Schema B NT vectors with ACh inversion.

Expected: All 5 monotonic activations produce 0/30 convergence with XOR
plateauing at 75% (3/4 correct). The barrier is a property of the monotonic
function CLASS, not a specific activation.

Results saved to: papers/emr-neuromodulation/results/monotonic_ablation/

Usage:
    python papers/emr-neuromodulation/scripts/runners/benchmark_monotonic_ablation.py
    python papers/emr-neuromodulation/scripts/runners/benchmark_monotonic_ablation.py \
        --seeds 3 --conditions uniform_tanh uniform_sigmoid  # smoke test
    python papers/emr-neuromodulation/scripts/runners/benchmark_monotonic_ablation.py --summary
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / 'results' / 'monotonic_ablation'
...
# ============================================================================
# Activation functions, all monotonic
# ============================================================================
...
def _identity(x):
    """Identity activation (linear, no nonlinearity)."""
    return x
...
def run_es(
    ...
    return {
        'experiment': 'E-S21_monotonic_ablation',
        'condition': condition,
...
def main():
    parser = argparse.ArgumentParser(
        description='E-S21: Monotonic activation ablation (H2 strengthening)')
    ...
    if args.summary:
        print("\n=== E-S21 Monotonic Activation Ablation — Summary ===\n")
    ...
    for condition in conditions:
        ...
        print(f"  Activation: {activation_name} (monotonic)")
        print(f"  Tasks: {TASK_NAMES} (2-input, 4 patterns each)")
        ...
        for seed in range(args.seeds):
            ...
            try:
                ...
                xor_acc = result['per_task_fitness'].get('xor', 0.0)
