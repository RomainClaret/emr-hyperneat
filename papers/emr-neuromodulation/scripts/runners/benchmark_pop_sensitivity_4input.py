#!/usr/bin/env python3
"""E-S4: Population Sensitivity at 4-Input with Per-Task Activation.

Tests whether the low success rate on 4-input tasks (13.3% at Pop=750) is
search-limited or structural by varying population size.

Tasks (4-input, 16 patterns each):
  - Parity-4: XOR(x1,x2,x3,x4), output 1 if odd number of 1s
  - AND-4: output 1 iff all 4 inputs = 1
  - OR-4: output 1 iff any input = 1

Conditions: per-task activation (sin for Parity-4, tanh for AND-4/OR-4)
Pop = {100, 300, 750, 1500} × 30 seeds = 120 runs

Architecture: MLP + (mu+lambda)-ES (consistent with E22/E33)
N_IN=4, N_HIDDEN=20, 200 gens.

Results saved to: papers/emr-neuromodulation/results/pop_sensitivity_4input/

Usage:
    python papers/emr-neuromodulation/scripts/runners/benchmark_pop_sensitivity_4input.py
    python papers/emr-neuromodulation/scripts/runners/benchmark_pop_sensitivity_4input.py \
        --seeds 3 --pop-sizes 100 750  # smoke test
    python papers/emr-neuromodulation/scripts/runners/benchmark_pop_sensitivity_4input.py --summary
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / 'results' / 'pop_sensitivity_4input'
...
INPUTS = jnp.array(list(iterproduct([0.0, 1.0], repeat=4)))  # (16, 4)

TASK_NAMES = ['parity4', 'and4', 'or4']

# Truth tables
def _parity4_targets():
    targets = []
    for bits in iterproduct([0.0, 1.0], repeat=4):
...
DEFAULT_POP_SIZES = [100, 300, 750, 1500]
...
def init_params(key: jax.Array, n_layers: int = 1) -> Dict[str, jnp.ndarray]:
    params = {}
    scale = 0.5
    n_keys = 4 * n_layers + 2
...
def init_population(key: jax.Array, pop_size: int, n_layers: int = 1) -> Dict[str, jnp.ndarray]:
...
def eval_single_multitask(params: Dict, n_layers: int = 1) -> Tuple[float, Dict[str, float]]:
    """Evaluate with per-task activation (always pertask for this experiment)."""
    per_task = {}
...
def run_es(
    ...
    for gen in range(generations):
        ...
        for i in range(pop_size):
            ...
            prod_fit, pt = eval_single_multitask(ind_params, n_layers)
    ...
    return {
        'condition': f'pertask_pop{pop_size}',
        'n_layers': n_layers,
...
def summarize_pop(results_dir: Path, pop_size: int):
    ...
    if parity_accs:
        print(f" | Parity-4 solved: {sum(1 for a in parity_accs if a >= 0.95)}/{len(parity_accs)}", end='')
    print()
...
def main():
    parser = argparse.ArgumentParser(
        description='E-S4: Population sensitivity at 4-input (per-task activation)')
    parser.add_argument('--seeds', type=int, default=30,
                        help='Number of seeds (default: 30)')
    parser.add_argument('--pop-sizes', nargs='+', type=int, default=None,
                        help='Population sizes (default: 100 300 750 1500)')
    ...
    if args.summary:
        print("\n=== E-S4 Pop Sensitivity 4-Input Summary ===\n")
    ...
    for pop_size in pop_sizes:
        ...
        print(f"\n{'='*60}")
        print(f"Pop={pop_size}, per-task activation ({args.seeds} seeds)")
        print(f"  Tasks: {TASK_NAMES} (4-input, 16 patterns each)")
        ...
        for seed in range(args.seeds):
            ...
            try:
                ...
                par_acc = result['per_task_fitness'].get('parity4', 0.0)
                print(f"  -> {status} (min_acc={result['min_task_accuracy']:.4f}, "
                      f"Parity-4={par_acc:.4f}, {result['runtime_seconds']:.1f}s)")
