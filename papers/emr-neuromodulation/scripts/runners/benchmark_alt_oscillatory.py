#!/usr/bin/env python3
"""E-S6: Alternative Oscillatory Activations, H2 Strengthening.

Tests whether H2 (oscillatory functions break the multi-task barrier) generalizes
beyond sin to the oscillatory CLASS, not just one specific function.

Conditions (5 × 30 seeds = 150 runs):
  1. uniform_cos, phase-shifted sin (should succeed)
  2. uniform_sin2x, double frequency sin (should succeed)
  3. uniform_gcu, x*cos(x), growing cosine unit (oscillatory but growing)
  4. uniform_dampened_sin, sin(x)/(1 + 0.1|x|), decaying oscillation
  5. uniform_tanh, negative control (known 0%)

Architecture: Direct-encoded MLP + (mu+lambda)-ES, consistent with validation experiments.
5 tasks (XOR, AND, OR, NAND, NOR), 2-input, N_HIDDEN=10, Pop=750, 100 gen.
Schema B NT vectors with ACh inversion.

Results saved to: papers/emr-neuromodulation/results/alt_oscillatory/

Usage:
    python papers/emr-neuromodulation/scripts/runners/benchmark_alt_oscillatory.py
    python papers/emr-neuromodulation/scripts/runners/benchmark_alt_oscillatory.py \
        --seeds 3 --conditions uniform_cos uniform_tanh  # smoke test
    python papers/emr-neuromodulation/scripts/runners/benchmark_alt_oscillatory.py --summary
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / 'results' / 'alt_oscillatory'
...
def main():
    parser = argparse.ArgumentParser(
        description='E-S6: Alternative oscillatory activations (H2 strengthening)')
    ...
    if args.summary:
        print("\n=== E-S6 Alternative Oscillatory Activations — Summary ===\n")
