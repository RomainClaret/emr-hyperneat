#!/usr/bin/env python3
"""E-S22: Per-Task Activation × Topology at 3-Input.

Completes the topology × activation × arity interaction matrix.
The paper establishes:
  - Topology neutral at 2-input (E-S5)
  - Topology transformative at 3-input with uniform tanh (E-S12: 20% FF → 80% recurrent)
  - Per-task activation achieves 100% at 3-input with feedforward (E-S9)

Missing cell: per-task activation + recurrence at 3-input.
Expected: Both 100% (per-task already saturates at feedforward), but convergence
speed may differ, recurrence could accelerate or slow down.

Conditions (2 × 30 seeds = 60 runs):
  1. pertask_feedforward, per-task activation, standard MLP
  2. pertask_full_recurrent, per-task activation, lateral connections, 3 recurrent steps

Architecture: Direct-encoded MLP + (mu+lambda)-ES.
5 tasks: Parity-3, AND-3, OR-3, NAND-3, NOR-3.
3-input, 8 patterns each, N_HIDDEN=20, Pop=750, 200 gen.
Schema B NT vectors with ACh inversion.

Results saved to: papers/emr-neuromodulation/results/pertask_topology_3input/

Usage:
    python papers/emr-neuromodulation/scripts/runners/benchmark_pertask_topology_3input.py
    python papers/emr-neuromodulation/scripts/runners/benchmark_pertask_topology_3input.py \
        --seeds 3 --conditions pertask_feedforward  # smoke test
    python papers/emr-neuromodulation/scripts/runners/benchmark_pertask_topology_3input.py --summary
"""
...
RESULTS_DIR = Path(__file__).resolve().parents[2] / 'results' / 'pertask_topology_3input'
...
# Parity-class tasks need oscillatory activation
PARITY_TASKS = {'parity3'}
...
# Schema B NT vectors mapped to 3-input task types
NT_VECTORS = {
    ...
    'nand3':   jnp.array([0.10, 0.90, 0.10, 0.0]),   # AND + inversion
    'nor3':    jnp.array([0.50, 0.50, 0.50, 0.0]),   # OR + inversion
}

# Per-task activation: sin for parity, tanh for threshold
TASK_ACTIVATIONS = {
    'parity3': 'sin',
    'and3': 'tanh',
    'or3': 'tanh',
...
ALL_CONDITIONS = ['pertask_feedforward', 'pertask_full_recurrent']
...
# Cache JIT-compiled forward functions per (activation, topology) pair
_FORWARD_CACHE = {}

def get_forward_fn(activation_name: str, topology: str):
    cache_key = (activation_name, topology)
    if cache_key not in _FORWARD_CACHE:
        act_fn = ACTIVATION_MAP[activation_name]
        if topology == 'feedforward':
            @jax.jit
            def _forward(params, inputs, nt):
                return _forward_neuromod(params, inputs, nt, act_fn, 'feedforward')
        else:
            @jax.jit
            def _forward(params, inputs, nt):
                return _forward_neuromod(params, inputs, nt, act_fn, 'full_recurrent')
        _FORWARD_CACHE[cache_key] = _forward
...
def eval_single_multitask(params: Dict, topology: str) -> Tuple[float, Dict[str, float]]:
    ...
    for task in TASK_NAMES:
        ...
        activation_name = TASK_ACTIVATIONS[task]

        forward_fn = get_forward_fn(activation_name, topology)
...
def run_es(
    ...
    topology = condition.replace('pertask_', '')
    ...
    return {
        'experiment': 'E-S22_pertask_topology_3input',
        'condition': condition,
        'topology': topology,
        'activation_mode': 'per_task',
...
def summarize_condition(results_dir: Path, condition: str):
    ...
    print(f"  {condition:<30}: {converged}/{total} ({rate:.1f}%)", end='')
...
def main():
    parser = argparse.ArgumentParser(
        description='E-S22: Per-task activation × topology at 3-input')
    ...
    if args.summary:
        print("\n=== E-S22 Per-Task × Topology 3-Input — Summary ===\n")
    ...
    for condition in conditions:
        ...
        print(f"  Topology: {topology}, Activation: per-task (sin/tanh)")
        ...
        print(f"  Task activations: {TASK_ACTIVATIONS}")
        print(f"{'='*60}")
