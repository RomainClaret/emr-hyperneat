"""Strategy 133S: Morphogen Sin Guaranteed Symmetric.

Morphogen-based with guaranteed sin and extreme protection.
Already has sin guarantee from dual - adds extreme protection.
"""
...
@dataclass
class MorphogenSinGuaranteedSymmetricState:
...
class MorphogenSinGuaranteedSymmetricStrategy(PaletteEvolutionStrategy):
    """Morphogen with guaranteed sin and extreme protection."""

    name = "morphogen_sin_guaranteed_symmetric"
    description = "Symmetric: Morphogen with guaranteed sin and extreme protection"

    def __init__(
        self,
        concentration_threshold: float = 0.35,
        guaranteed_concentration: float = 0.9,
        ...
        self.guaranteed_concentration = guaranteed_concentration
    ...
    def _apply_floors(self, act_conc, agg_conc, act_aff, agg_aff):
        # Concentration floors
        new_act_conc = act_conc.at[SIN_IDX].set(jnp.maximum(act_conc[SIN_IDX], self.guaranteed_concentration))
        new_agg_conc = agg_conc
        for idx in [MAX_IDX, MIN_IDX]:
            new_agg_conc = new_agg_conc.at[idx].set(jnp.maximum(new_agg_conc[idx], self.guaranteed_concentration))
        ...
        return new_act_conc, new_agg_conc, new_act_aff, new_agg_aff

    def initialize(self, config: Dict[str, Any], seed: int) -> MorphogenSinGuaranteedSymmetricState:
        key = jax.random.PRNGKey(seed + 133300)
        ...
        act_conc = jnp.ones(NUM_ACTIVATIONS) * 0.4
        agg_conc = jnp.ones(NUM_AGGREGATIONS) * 0.4
        act_affinity = jnp.ones(NUM_ACTIVATIONS) * 0.5
        agg_affinity = jnp.ones(NUM_AGGREGATIONS) * 0.5

        act_conc, agg_conc, act_affinity, agg_affinity = self._apply_floors(
            act_conc, agg_conc, act_affinity, agg_affinity
        )
        ...
        return MorphogenSinGuaranteedSymmetricState(
    ...
    def post_generation_update(self, state, generation, best_fitness, prev_best_fitness, population_data=None):
        ...
        # Update concentrations and affinities
        delta = improvement if improvement > 0 else -0.01
        new_act_conc = state.act_concentrations + delta * 0.1 * state.act_mask
        new_agg_conc = state.agg_concentrations + delta * 0.1 * state.agg_mask
        ...
        new_act_conc, new_agg_conc, new_act_aff, new_agg_aff = self._apply_floors(
            new_act_conc, new_agg_conc, new_act_aff, new_agg_aff
        )
        ...
        new_state = MorphogenSinGuaranteedSymmetricState(
