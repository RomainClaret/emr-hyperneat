"""Strategy 88: GRN Fixed Dual (Reduced Coupling for Better Sin Retention).

Fixes the over-stabilization issue in Strategy 78 (GRN Agg Discovery) which
achieved 0% sin retention due to excessive coupling strength.

Key fixes:
- REDUCED sin_to_max/min coupling: 0.15 (was 0.4) - allows exploration
- SOFTER Hill coefficient: 1.2 (was 2.0) - gentler expression dynamics
- FASTER expression decay: 0.85 (was 0.9) - reduces lock-in effect

Bio inspiration: Gene regulatory networks need balanced coupling to avoid
homeostatic lock-in. Excessive coupling prevents adaptive exploration.

Expected: Positive sin retention (vs 0% in strategy 78).
"""
...
class GRNFixedDualStrategy(PaletteEvolutionStrategy):
    """Dual palette evolution with fixed (reduced) sin-extreme coupling.

    Addresses over-stabilization in strategy 78 by:
    1. Reducing coupling strength (0.4 -> 0.15)
    2. Softening Hill coefficient (2.0 -> 1.2)
    3. Faster expression decay (0.9 -> 0.85)
    """

    name = "grn_fixed_dual"
    description = "Dual: GRN with fixed (reduced) coupling for sin retention"

    def __init__(
        self,
        # Expression dynamics - FIXED VALUES
        basal_expression: float = 0.1,
        hill_coefficient: float = 1.2,  # FIXED: Was 2.0 (too aggressive)
        agg_hill_coefficient: float = 1.0,  # FIXED: Was 1.5
        half_max_constant: float = 0.5,
        expression_decay: float = 0.85,  # FIXED: Was 0.9 (too slow)
        # Regulatory network
        initial_regulation_strength: float = 0.3,
        regulation_learning_rate: float = 0.10,  # Slightly higher
        regulation_decay: float = 0.96,  # Slightly faster decay
        regulation_max: float = 1.5,
        network_sparsity: float = 0.3,
        # Activation/Inhibition
        activation_bias: float = 0.6,
        inhibition_strength_factor: float = 0.8,
        # Cross-domain - REDUCED COUPLING (key fix)
        cross_regulation_sparsity: float = 0.15,
        cross_regulation_strength: float = 0.15,  # Reduced from 0.2
        sin_to_max_coupling: float = 0.15,  # FIXED: Was 0.4 (caused over-stabilization)
        sin_to_min_coupling: float = 0.15,  # FIXED: Was 0.4
        sin_extreme_lr_multiplier: float = 1.3,  # Slightly reduced
        # Expression threshold
        expression_threshold: float = 0.35,  # Lowered for more exploration
        agg_expression_threshold: float = 0.30,  # Lowered
        # Palette composition
        ...
        """Initialize GRN Fixed Dual strategy with reduced coupling."""
        # Expression
        ...
        # Cross-domain - REDUCED sin-extreme coupling
    ...
    def _initialize_cross_regulation(
        ...
        """Initialize cross-domain regulation with REDUCED sin-extreme coupling."""
        ...
        # REDUCED sin-extreme coupling (key fix for sin retention)
        sin_idx = 4
        max_idx = 2
        min_idx = 3
    ...
    def initialize(self, config: Dict[str, Any], seed: int) -> Dict[str, Any]:
        """Initialize state with dual regulatory networks and reduced coupling."""
        initial_act = config.get('initial_palette', self.initial_act_palette)
        initial_agg = config.get('initial_agg_palette', self.initial_agg_palette)

        key = jax.random.PRNGKey(seed + 880000)
    ...
    def _update_agg_expression(
        ...
        """Update aggregation expression with REDUCED cross-domain sin influence."""
        ...
        for j in range(NUM_AGGREGATIONS):
            ...
            # REDUCED: Extra sin->extreme coupling for max/min (smaller boost)
            if j in CORE_EXTREME_AGGS:
                if j == 2:  # max
                    activator_sum += sin_expr * self.sin_to_max_coupling * 0.3  # Was 0.5
                elif j == 3:  # min
                    activator_sum += sin_expr * self.sin_to_min_coupling * 0.3  # Was 0.5
    ...
    def _update_cross_regulation(
        ...
        """Update cross-domain regulation with REDUCED sin-extreme learning."""
        ...
        if improvement > 0:
            ...
            # REDUCED sin-extreme learning (prevents over-stabilization)
            sin_active = act_mask[4] > 0.5
            if sin_active:
                for agg_idx in CORE_EXTREME_AGGS:
                    if agg_mask[agg_idx] > 0.5:
                        boost = self.regulation_learning_rate * improvement * self.sin_extreme_lr_multiplier * 0.7  # Reduced
    ...
    def post_generation_update(
        ...
        """Update with GRN dynamics and REDUCED sin-extreme coupling."""
