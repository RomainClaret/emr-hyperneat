"""Strategy 85: Tag+Reinforcement Dual Hybrid.
...
Key innovation: Cross-capture requires BOTH tag AND high affinity, combining
temporal importance (tagging) with cross-domain synergy (reinforcement).

Bio inspiration: Synaptic consolidation is modulated by neuromodulatory signals
that track global reward - tagging marks candidates, reinforcement confirms importance.

Expected: 100% sin retention + improved Parity-5 solving.
"""
...
class TagReinforcementDualStrategy(PaletteEvolutionStrategy):
    """Dual palette evolution combining tagging and cross-domain reinforcement.
    ...
    Critical interaction: Cross-capture requires BOTH domains tagged AND
    high cross-affinity, ensuring only synergistic pairs are consolidated.
    """

    name = "tag_reinforcement_dual"
    description = "Dual: Tag-and-capture + cross-domain reinforcement hybrid"
    ...
    def __init__(
        ...
        reinforcement_multiplier: float = 2.0,  # When BOTH domains change
        affinity_protection_threshold: float = 0.6,
        affinity_protection_strength: float = 0.5,
        # === Combined: Cross-capture threshold ===
        cross_capture_affinity_threshold: float = 0.65,  # Need high affinity + tag
        # === Mutation parameters ===
        ...
        """Initialize Tag+Reinforcement hybrid strategy."""
        ...
        # Combined
        self.cross_capture_affinity_threshold = cross_capture_affinity_threshold
    ...
    def initialize(self, config: Dict[str, Any], seed: int) -> Dict[str, Any]:
        """Initialize state with tagging and reinforcement tracking."""
        ...
        # Cross-domain affinity matrix (from reinforcement)
        cross_affinity = jnp.ones((NUM_ACTIVATIONS, NUM_AGGREGATIONS)) * 0.5

        # Track previous palette state for reinforcement
        ...
        return {
            ...
            # Tracking
            'capture_events': 0,
            'cross_capture_events': 0,  # Captures requiring both tag + affinity
            'reinforcement_events': 0,
            # General state
            'rng_key': jax.random.PRNGKey(seed + 850000),
            'generation': 0,
            'stagnation_count': 0,
    ...
    def _update_cross_affinity_reinforced(
        ...
        if improvement > 0:
            ...
            # Learning rate with reinforcement multiplier
    ...
    def _attempt_capture_combined(
        ...
        # Standard capture from tag history
        ...
        # CROSS-CAPTURE: Requires BOTH high tag AND high affinity
        for hist_gen, hist_act_tags, hist_agg_tags in tag_history:
            if generation - hist_gen <= self.capture_window:
                for i in range(NUM_ACTIVATIONS):
                    for j in range(NUM_AGGREGATIONS):
                        # Both must be tagged AND have high affinity
                        act_tagged = hist_act_tags[i] > self.tag_threshold * 0.8
                        agg_tagged = hist_agg_tags[j] > self.agg_tag_threshold * 0.8
                        high_affinity = cross_affinity[i, j] > self.cross_capture_affinity_threshold
    ...
    def _mutate_act_palette(
        ...
        """Mutate activation palette with tag and affinity protection."""
        new_mask = mask.copy()
        activated = []
        deactivated = []
        ...
        for i in range(NUM_ACTIVATIONS):
            ...
            if mask[i] < 0.5:  # Inactive
                ...
                # AFFINITY PROTECTION (from reinforcement)
                if max_affinity > self.affinity_protection_threshold:
    ...
    def _mutate_agg_palette(
        ...
        """Mutate aggregation palette with tag and affinity protection."""
        new_mask = mask.copy()
        activated = []
        deactivated = []
        ...
        for j in range(NUM_AGGREGATIONS):
            ...
            if mask[j] < 0.5:  # Inactive
                ...
                if max_affinity > self.affinity_protection_threshold:
                    activate_rate *= 1.5
                # Extra boost for extreme aggs with high sin affinity
                if j in CORE_EXTREME_AGGS and sin_affinity > 0.6:
                    activate_rate *= 1.3
                if p < activate_rate:
    ...
    def post_generation_update(
        ...
        """Update with combined tag+reinforcement mechanisms."""
        ...
        if should_mutate:
            new_act_mask, _ = self._mutate_act_palette(
                k_act, state['act_mask'], new_act_captured, new_cross_affinity
            )
            new_agg_mask, _ = self._mutate_agg_palette(
                k_agg, state['agg_mask'], new_agg_captured, new_cross_affinity, state['act_mask']
            )
