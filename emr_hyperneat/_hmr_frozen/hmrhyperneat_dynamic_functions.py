"""HMR-HyperNEAT with Dynamic Functions - Evolved Activation/Aggregation Support.

This is an EXPERIMENTAL extension of HMR-HyperNEAT that allows evolution to choose
activation and aggregation functions for substrate nodes. Part of a progressive
A/B testing study to understand what works and why.

Dynamic Function Modes
======================
...
# Pre-defined palette configurations for testing hypothesis that palette choice matters
# Key insight: default palette [0,1,2,3] doesn't include sin (4), which is optimal for parity
PALETTE_CONFIGS = {
...
# ============================================================================
# Palette Evolution (Meta-Evolution of Activation Selection)
# ============================================================================
# These functions support evolving which activation functions are available
# to the CPPN, allowing automatic discovery of problem-optimal palettes.
# Pre-test showed: sin_only achieves 100% solve on Parity-4 in 3.3 gens,
# while default (tanh,sigmoid,relu,identity) achieves 0% solve rate.

NUM_ACTIVATIONS = len(ACTIVATION_LIST)  # 13 total activations


def mutate_palette_mask(
    key: jax.random.PRNGKey,
    mask: jnp.ndarray,
    mutation_rate: float = 0.1,
    min_active: int = 2,
) -> jnp.ndarray:
    """Mutate palette mask by flipping bits with given probability.

    Args:
        key: JAX random key
        mask: Current palette mask (13,), values in [0, 1]
        mutation_rate: Probability of flipping each bit
        min_active: Minimum number of activations that must remain active

    Returns:
        Mutated palette mask
    """
    # Generate flip mask
    ...
    # Ensure minimum activations are active
    # If too few, keep original mask
    active_count = jnp.sum(new_mask > 0.5)
    new_mask = jnp.where(active_count >= min_active, new_mask, mask)

    return new_mask


def mask_to_palette(mask: jnp.ndarray) -> jnp.ndarray:
    """Convert palette mask to list of active indices.

    Args:
        mask: Palette mask (13,), values in [0, 1]

    Returns:
        Array of active activation indices
    """
    return jnp.where(mask > 0.5)[0].astype(jnp.int32)


def create_initial_palette_mask(config: str | list) -> jnp.ndarray:
    """Create initial palette mask from config.

    Args:
        config: 'full' (all), 'default' (first 4), or list of indices

    Returns:
        Palette mask (13,) with 1.0 for active, 0.0 for disabled
    """
    mask = jnp.zeros(NUM_ACTIVATIONS, dtype=jnp.float32)

    if config == 'full':
        mask = jnp.ones(NUM_ACTIVATIONS, dtype=jnp.float32)
    elif config == 'default':
        # Default: tanh, sigmoid, relu, identity (indices 0-3)
...
class HMRHyperNEAT(BaseAlgorithm):
    ...
    def __init__(self, name: str = 'hmr-hyperneat',
        ...
        # =====================================================================
        # Live Palette Strategy Integration
        # =====================================================================
        # Enables bio-strategies to inject discovered palettes into running evolution.
        # Addresses the "discovery ≠ usage" gap where strategies discover sin but
        # networks don't use it because CPPN weights are locked to old palette indices.
        self._palette_strategy = None  # Strategy instance (set via set_palette_strategy)
        self._palette_strategy_enabled = False  # Whether live updates are active
        self._palette_perturbation_rate = 0.15  # Fraction of population to perturb
        self._palette_perturbation_strength = 0.3  # How much to shift weights
        self._generation_count = 0  # Track generations for strategy updates
        self._fitness_history = []  # Track fitness for strategy decisions
    ...
    def create_config(self, params: Dict[str, Any]) -> Any:
        ...
        # - 'global': All hidden nodes use same configurable activation
        # - 'cppn_output': CPPN outputs activation index per node
        #                  Supports 'palette' config to control which activations are available
        # - 'weight_interpretation': Derive activation from weight patterns
        ...
        # For 'cppn_output' mode: custom palette (which specific activations to use)
        ...
        # =====================================================================
        # PALETTE EVOLUTION: Let evolution discover optimal activation palettes
        # =====================================================================
        # When enabled, the palette mask is evolved alongside the CPPN.
        # This allows automatic discovery of problem-specific activation functions.
        # Pre-test showed: sin_only palette achieves 100% solve in 3.3 gens on Parity-4,
        # while default palette achieves 0% solve rate.
        palette_evolution_config = dynamic_funcs.get('palette_evolution', {})
        self.df_palette_evolution_enabled = palette_evolution_config.get('enabled', False)
        self.df_palette_evolution_config = {
            'mutation_rate': palette_evolution_config.get('mutation_rate', 0.1),
            'min_active': palette_evolution_config.get('min_active', 2),  # At least 2 activations
            'initial_mask': palette_evolution_config.get('initial_mask', 'full'),  # 'full', 'default', or list
        }
        # Initialize palette mask for evolution (13 activations)
        # 1.0 = active, 0.0 = disabled
        self._palette_mask = None  # Populated in initialize()
        self._palette_rng_key = None  # For reproducible mutations
        ...
        # LIVE PALETTE STRATEGY INTEGRATION
        # =====================================================================
        # Fixes the "discovery ≠ usage" gap where bio-strategies discover sin but
        # networks don't use it because CPPN weights map to old palette positions.
        #
        # When enabled, the algorithm calls the strategy's get_active_palette() each
        # generation and updates self.df_palette dynamically. When the palette expands,
        # a fraction of the population has their CPPN activation outputs perturbed to
        # explore the newly added functions.
        live_strategy_config = dynamic_funcs.get('live_strategy', {})
        self._palette_strategy_enabled = live_strategy_config.get('enabled', False)
        self._palette_perturbation_rate = live_strategy_config.get('perturbation_rate', 0.15)
        self._palette_perturbation_strength = live_strategy_config.get('perturbation_strength', 0.3)
        # Decay perturbation over time (0.0 = no decay, 1.0 = linear decay to 0)
        self._palette_perturbation_decay = live_strategy_config.get('perturbation_decay', 0.0)
        ...
        # Log palette evolution if enabled
        if self.df_palette_evolution_enabled:
            print(f"[Palette Evolution] ENABLED - activations will be evolved")
            print(f"[Palette Evolution] Config: mutation_rate={self.df_palette_evolution_config['mutation_rate']}, "
                  f"min_active={self.df_palette_evolution_config['min_active']}, "
                  f"initial={self.df_palette_evolution_config['initial_mask']}")
        ...
        # Determine CPPN output count based on dynamic_functions mode
        # Mode='cppn_output' needs 2 outputs: [weight, activation_raw]
        self.cppn_num_outputs = 2 if self.dynamic_functions_mode == 'cppn_output' else 1
    ...
    def initialize(self, config: Any, problem: Any, seed: int = 42) -> Any:
        ...
        # Initialize palette evolution state if enabled
        if self.df_palette_evolution_enabled:
            initial_mask_config = self.df_palette_evolution_config['initial_mask']
            self._palette_mask = create_initial_palette_mask(initial_mask_config)
            self._palette_rng_key = jax.random.PRNGKey(seed + 12345)  # Separate key for palette
            # Convert mask to palette for use in evaluation
            self.df_palette = mask_to_palette(self._palette_mask)
            print(f"[Palette Evolution] Initialized with mask: {list(self._palette_mask.astype(int))}")
            print(f"[Palette Evolution] Initial palette: {list(self.df_palette)} -> "
                  f"{[ACTIVATION_LIST[i] for i in self.df_palette]}")
    ...
    def set_palette_strategy(self, strategy: Any) -> None:
        """Set the palette strategy for live palette updates.

        This enables bio-inspired strategies (Ant Colony, Critical Period, etc.)
        to inject discovered palettes into running evolution. When the strategy
        discovers new beneficial functions (like sin), they are immediately added
        to the algorithm's palette and the population is perturbed to explore them.

        Args:
            strategy: A palette strategy instance with these methods:
                - initialize(): Called once at start
                - get_active_palette() -> List[int]: Returns current palette
                - post_generation_update(generation, best_fitness, ...): Called each gen

        Example:
            from palette_strategies.strategy_32_ant_colony_pheromone import AntColonyPheromoneStrategy

            algo = HMRHyperNEAT()
            strategy = AntColonyPheromoneStrategy(initial_palette=[0, 1, 2, 3])
            algo.set_palette_strategy(strategy)

            # Strategy will inject sin when discovered via pheromone mechanism
        """
        self._palette_strategy = strategy
        self._palette_strategy_enabled = True
        self._generation_count = 0
        self._fitness_history = []

        # Initialize strategy if not already initialized
        # Note: Many strategies are initialized externally before being passed here
        # We only call initialize if it exists and accepts no arguments
        if hasattr(strategy, 'initialize'):
            import inspect
            sig = inspect.signature(strategy.initialize)
            # Only call if it can accept no arguments (already initialized strategies)
            if all(p.default != inspect.Parameter.empty or p.kind == inspect.Parameter.VAR_POSITIONAL
                   for p in sig.parameters.values()):
                try:
                    strategy.initialize()
                except TypeError:
                    pass  # Strategy already initialized

        # Set initial palette from strategy
        if hasattr(strategy, 'get_active_palette'):
            initial_palette = strategy.get_active_palette()
            self.df_palette = jnp.array(initial_palette, dtype=jnp.int32)
            print(f"[Live Strategy] Initialized with palette: {list(initial_palette)} -> "
                  f"{[ACTIVATION_LIST[i] for i in initial_palette]}")

    def update_palette_from_strategy(self, state: Any, best_fitness: float) -> Any:
        """Update palette based on strategy recommendations.

        Called each generation after fitness evaluation. If the strategy suggests
        a new palette (e.g., after discovering sin), updates self.df_palette and
        optionally perturbs the population to explore newly added functions.
        ...
        Returns:
            Modified state (with perturbed population if palette expanded)
        """
        if not self._palette_strategy_enabled or self._palette_strategy is None:
            return state

        # Update strategy with generation info
        self._generation_count += 1
        self._fitness_history.append(best_fitness)

        if hasattr(self._palette_strategy, 'post_generation_update'):
            self._palette_strategy.post_generation_update(
                generation=self._generation_count,
                best_fitness=best_fitness,
                fitness_history=self._fitness_history,
            )

        # Get updated palette from strategy
        if hasattr(self._palette_strategy, 'get_active_palette'):
            new_palette = self._palette_strategy.get_active_palette()

            # Check if palette changed
            # Use tolist() for JAX arrays to get proper Python list
            if self.df_palette is not None:
                current_palette = self.df_palette.tolist() if hasattr(self.df_palette, 'tolist') else list(self.df_palette)
            else:
                current_palette = []
            if new_palette != current_palette:
                old_size = len(current_palette)
                new_size = len(new_palette)

                # Update palette
                self.df_palette = jnp.array(new_palette, dtype=jnp.int32)

                # Find newly added functions
                new_funcs = set(new_palette) - set(current_palette)
                if new_funcs:
                    new_func_names = [ACTIVATION_LIST[i] for i in new_funcs]
                    print(f"[Live Strategy] Gen {self._generation_count}: "
                          f"Added functions {new_func_names} -> palette {new_palette}")

                # If palette expanded, perturb population to explore new functions
                if new_size > old_size and self._palette_perturbation_rate > 0:
                    state = self._perturb_activation_outputs(
                        state, old_size, new_size
                    )

        return state

    def _perturb_activation_outputs(
        self,
        state: Any,
        old_palette_size: int,
        new_palette_size: int,
    ) -> Any:
        """Perturb CPPN activation outputs to explore newly added functions.

        When the palette expands (e.g., from 4 to 5 after sin is discovered),
        the CPPN weights that determine activation indices are locked to the
        old range [0, 3]. This method perturbs a fraction of the population's
        activation output weights to push them into the new range, enabling
        exploration of the newly added functions.

        The perturbation uses continuous_to_palette_index's mapping:
        - Palette size 4: tanh output [-1,1] maps to bins [0,1,2,3]
        - Palette size 5: tanh output [-1,1] maps to bins [0,1,2,3,4]

        To reach bin 4, we need tanh(x) > 0.6, so we shift activation outputs
        toward positive values for a fraction of the population.

        Args:
            state: Algorithm state containing population
            old_palette_size: Size of palette before expansion
            new_palette_size: Size of palette after expansion

        Returns:
            Modified state with perturbed population
        """
        # Calculate effective perturbation rate (with optional decay)
        effective_rate = self._palette_perturbation_rate
        if self._palette_perturbation_decay > 0 and self._generation_count > 0:
            # Linear decay over 100 generations
            decay_factor = max(0.0, 1.0 - self._palette_perturbation_decay *
                              self._generation_count / 100.0)
            effective_rate *= decay_factor

        if effective_rate <= 0:
            return state

        # Get population from state
        # TensorNEAT state structure: state.pop_nodes and state.pop_conns
        try:
            nodes = state.pop_nodes
            conns = state.pop_conns
        except (KeyError, AttributeError):
            print("[Live Strategy] Warning: Could not access population, skipping perturbation")
            return state

        if nodes is None:
            return state

        pop_size = nodes.shape[0]

        # Determine which individuals to perturb
        num_to_perturb = max(1, int(pop_size * effective_rate))

        # Use state's random key to select which to perturb
        key = state.randkey
        key, subkey = jax.random.split(key)
        indices_to_perturb = jax.random.choice(
            subkey, pop_size, shape=(num_to_perturb,), replace=False
        )

        # For perturbed individuals, we need to modify their CPPN output node weights
        # to shift activation outputs toward the new palette bins.
        #
        # The activation output is the second CPPN output (index 1 if 2 outputs).
        # We want to add a positive bias to push outputs into the higher palette bins.
        #
        # Strategy: Add perturbation_strength to the output node's bias or weights
        # This shifts the activation_raw output toward positive values, which maps
        # to higher palette indices.

        # Get CPPN output node indices (depends on TensorNEAT's genome structure)
        # In TensorNEAT, nodes array has shape (pop_size, max_nodes, node_features)
        # where node_features typically includes [bias, ...] or similar

        # add perturbation to all node biases for selected individuals
        # This is a simple approach - more targeted perturbation would modify
        # only the activation output pathway
        perturbation = self._palette_perturbation_strength

        # Create perturbation mask
        perturb_mask = jnp.zeros(pop_size, dtype=jnp.float32)
        perturb_mask = perturb_mask.at[indices_to_perturb].set(1.0)

        # Apply perturbation to node biases (assuming bias is in column 0)
        # This shifts outputs toward positive, reaching higher palette bins
        if nodes.ndim >= 3:
            # Shape: (pop_size, max_nodes, features)
            # Add perturbation to bias column for selected individuals
            bias_perturbation = perturb_mask[:, None, None] * perturbation
            # Only perturb if there's a bias dimension
            if nodes.shape[2] > 0:
                nodes = nodes.at[:, :, 0].add(bias_perturbation[:, :, 0])

        # Update state with perturbed population
        state = state.update(pop_nodes=nodes, pop_conns=conns, randkey=key)

        print(f"[Live Strategy] Perturbed {num_to_perturb}/{pop_size} individuals "
              f"(rate={effective_rate:.2%}) to explore new palette bins")
    ...
    def run_generation_verbose(
        ...
        # =====================================================================
        # STEP 6b: Live Palette Strategy Update
        # =====================================================================
        # Update palette based on bio-strategy recommendations.
        # This must happen AFTER fitness evaluation but BEFORE next generation.
        if self._palette_strategy_enabled and self._palette_strategy is not None:
            t6b = time.perf_counter()
            # Get best fitness for strategy decision (single JAX op, minimal overhead)
            best_fitness_for_strategy = float(jnp.max(fitnesses))
            new_state = self.update_palette_from_strategy(new_state, best_fitness_for_strategy)
            step_timings['step6b_palette_update'] = time.perf_counter() - t6b
        ...
        # =====================================================================
        # PALETTE EVOLUTION: Elite-guided palette mutation
        # =====================================================================
        # Strategy: Keep track of best fitness per palette, only adopt mutations
        # that improve fitness. This creates selection pressure on palettes.
        if self.df_palette_evolution_enabled and self._palette_mask is not None:
            current_best = metrics.best_fitness

            # Initialize tracking on first generation
            if not hasattr(self, '_palette_best_fitness'):
                self._palette_best_fitness = current_best
                self._palette_stagnation_count = 0

            # Check if current palette is still the best
            if current_best > self._palette_best_fitness:
                # Current palette is good, keep it and update best
                self._palette_best_fitness = current_best
                self._palette_stagnation_count = 0
            else:
                # No improvement, increment stagnation
                self._palette_stagnation_count += 1

            # Only mutate if stagnating (no improvement for N generations)
            stagnation_threshold = 5  # Try mutation after 5 gens of no improvement
            if self._palette_stagnation_count >= stagnation_threshold:
                self._palette_rng_key, subkey = jax.random.split(self._palette_rng_key)
                old_mask = self._palette_mask
                self._palette_mask = mutate_palette_mask(
                    subkey,
                    self._palette_mask,
                    mutation_rate=self.df_palette_evolution_config['mutation_rate'],
                    min_active=self.df_palette_evolution_config['min_active'],
                )
                # Update palette from mask
                self.df_palette = mask_to_palette(self._palette_mask)
                # Reset stagnation counter
                self._palette_stagnation_count = 0

                # Log if palette changed
                if not jnp.allclose(old_mask, self._palette_mask):
                    active_names = [ACTIVATION_LIST[i] for i in self.df_palette]
                    metrics.custom_metrics['palette_changed'] = True
                    metrics.custom_metrics['palette'] = list(map(int, self.df_palette))
                    metrics.custom_metrics['palette_names'] = active_names
                    metrics.custom_metrics['palette_mutation_reason'] = 'stagnation'
    ...
    def run_until_threshold(
        ...
        Note:
            Live palette strategy updates are NOT supported in this
            method because jax.lax.while_loop requires pure JAX operations with no
            Python side effects. Use run_generation_verbose() with manual loops for
            live palette integration.
    ...
    # ========================================================================
    # Continual Learning Support Methods
    # ========================================================================
    # These methods enable TRUE continual learning with genome transfer,
    # re-evaluation, and topology freezing.

    def get_best_genome_with_fitness(
        self, state: Any, fitnesses: jnp.ndarray
    ) -> Tuple[jnp.ndarray, jnp.ndarray, float, int]:
        """Extract the best genome from population based on fitness.

        Args:
            state: Current algorithm state
            fitnesses: Fitness array from last generation, shape (pop_size,)

        Returns:
            Tuple of (best_nodes, best_conns, best_fitness, best_idx)
            - best_nodes: CPPN nodes array, shape (max_nodes, node_attrs)
            - best_conns: CPPN connections array, shape (max_conns, conn_attrs)
            - best_fitness: float, fitness value of best individual
            - best_idx: int, index of best individual in population
        """
        if not hasattr(self, 'neat_algo') or self.neat_algo is None:
            raise RuntimeError("Algorithm not initialized - call initialize() first")

        # Get population
        pop = self.neat_algo.ask(state)
        pop_nodes, pop_conns = pop[0], pop[1]
        ...
        return best_nodes, best_conns, best_fitness, best_idx

    def inject_genome_into_population(
        self,
        state: Any,
        genome_nodes: jnp.ndarray,
        genome_conns: jnp.ndarray,
        num_copies: int = 1,
    ) -> Any:
        """Inject a genome into the population (warm-start for continual learning).

        Replaces the first `num_copies` individuals in the population with
        copies of the provided genome. This enables transferring knowledge
        from a previous task to seed the next task's evolution.

        Args:
            state: Current algorithm state
            genome_nodes: CPPN nodes to inject, shape (max_nodes, node_attrs)
            genome_conns: CPPN connections to inject, shape (max_conns, conn_attrs)
            num_copies: Number of population slots to fill with this genome (default 1)

        Returns:
            Updated state with injected genome(s)
        """
        if not hasattr(self, 'neat_algo') or self.neat_algo is None:
            raise RuntimeError("Algorithm not initialized - call initialize() first")
        ...
        # Limit copies to population size
        num_copies = min(num_copies, pop_size)

        # Inject genome into first num_copies slots
        for i in range(num_copies):
            pop_nodes = pop_nodes.at[i].set(genome_nodes)
            pop_conns = pop_conns.at[i].set(genome_conns)
    ...
    def evaluate_genome_on_problem(
        self,
        state: Any,
        genome_nodes: jnp.ndarray,
        genome_conns: jnp.ndarray,
        ...
        """Evaluate a single genome on a problem without evolution.

        Uses full HMR substrate query: CPPN → substrate → forward pass.
        This enables re-evaluating saved genomes on previous tasks to
        measure catastrophic forgetting.

        Args:
            state: Algorithm state (for CPPN forward function)
            genome_nodes: CPPN nodes, shape (max_nodes, node_attrs)
            genome_conns: CPPN connections, shape (max_conns, conn_attrs)
            problem: Problem instance with get_data() method

        Returns:
            Fitness value (float) for this genome on the problem
        """
        if not hasattr(self, 'neat_algo') or self.neat_algo is None:
            raise RuntimeError("Algorithm not initialized - call initialize() first")

        # Transform single genome to network-ready form
        # Need to add batch dimension for transform
        genome_nodes_batch = genome_nodes[None, :, :]  # (1, max_nodes, attrs)
        genome_conns_batch = genome_conns[None, :, :]  # (1, max_conns, attrs)

        # Transform CPPN
        cppn_transformed = self._compiled_transform_batch(
            state, (genome_nodes_batch, genome_conns_batch)
        )
        ...
        # Generate substrate coordinates for this problem
        ...
        input_coords = jnp.array(input_coords, dtype=jnp.float32)

        output_coords = []
        for i in range(n_outputs):
            x = -1.0 + 2.0 * i / max(n_outputs - 1, 1) if n_outputs > 1 else 0.0
            output_coords.append([x, 1.0])
        output_coords = jnp.array(output_coords, dtype=jnp.float32)

        # Query CPPN to build substrate weights
        # Shape: (1, num_inputs, total_positions)
        input_all_weights = batch_query_population_multi_source(
            state, cppn_transformed, input_coords,
            all_positions, True, self._jitted_cppn_forward
        )

        # Shape: (1, num_outputs, total_positions)
        output_all_weights = batch_query_population_multi_source(
            state, cppn_transformed, output_coords,
            all_positions, False, self._jitted_cppn_forward
        )

        # Build weight matrices (same as run_generation)
        max_weight = self.max_weight
        weight_thresh = 0.1
        ...
        W1 = W1_raw * (jnp.abs(W1_raw) > weight_thresh)
        W2 = jnp.transpose(W2_raw * (jnp.abs(W2_raw) > weight_thresh), (0, 2, 1))

        # Get problem data
        data = problem.get_data()
        inputs_list = [jnp.array(d[0], dtype=jnp.float32) for d in data]
        targets_list = [jnp.array(d[1], dtype=jnp.float32) for d in data]
        inputs_batch = jnp.stack(inputs_list)  # (num_samples, input_dim)
        targets_batch = jnp.stack(targets_list)  # (num_samples, output_dim)

        # Forward pass through substrate (single genome, batch of samples)
        W1_single = W1[0]  # (num_inputs, total_positions)
        W2_single = W2[0]  # (total_positions, num_outputs)

        hidden = jnp.tanh(jnp.matmul(inputs_batch, W1_single))
        outputs = jax.nn.sigmoid(jnp.matmul(hidden, W2_single))
    ...
    def create_state_for_new_problem(
        self,
        problem: Any,
        seed: int,
        inherit_genome: Optional[Tuple[jnp.ndarray, jnp.ndarray]] = None,
    ) -> Any:
        """Create a fresh state for a new problem, optionally inheriting a genome.

        This is used in continual learning to set up evolution for a new task
        while optionally seeding the population with a genome from a previous task.

        Args:
            problem: New problem instance
            seed: Random seed for initialization
            inherit_genome: Optional tuple of (nodes, conns) to inject

        Returns:
            Initialized state, ready for evolution
        """
        # Create config for new problem
        n_inputs = problem.input_shape[0]
        n_outputs = problem.output_shape[0]
        ...
        # Build config preserving current settings
        algo_config = {
            'algorithm_params': {
                'hmrhyperneat': {
                    'hmr_hyperneat': {
                        ...
                        'variance_threshold': self.variance_threshold,
                        'dynamic_functions': {
                            'mode': self.dynamic_functions_mode,
                            'palette': list(self.df_palette) if hasattr(self, 'df_palette') else [0, 1, 2, 3],
                        },
                    },
                    'substrate': {
                    ...
                    'neat': {
                        'pop_size': self.pop_size if hasattr(self, 'pop_size') else 150,
                    },
                }
            }
        }

        config = self.create_config(algo_config)
        state = self.initialize(config, problem, seed=seed)

        # Inject inherited genome if provided
        if inherit_genome is not None:
            genome_nodes, genome_conns = inherit_genome
            state = self.inject_genome_into_population(state, genome_nodes, genome_conns)

        return state

    def freeze_topology(self, state: Any) -> Any:
        """Freeze CPPN topology, allowing only weight mutations.

        Used for Mode B continual learning to test whether preserving
        topology reduces catastrophic forgetting.

        Implementation:
        - Zeros out structural mutation probabilities (node_add, node_delete,
          conn_add, conn_delete) in TensorNEAT's DefaultMutation
        - Re-compiles _compiled_tell to capture the new zero probabilities
        - Weight mutations (mutate_values) continue to work

        Args:
            state: Current algorithm state

        Returns:
            Modified state with structural mutations disabled
        """
        if not hasattr(self, 'neat_algo') or self.neat_algo is None:
            raise RuntimeError("Cannot freeze topology: neat_algo not initialized")

        # Access the mutation object from NEAT algorithm's genome
        mutation = self.neat_algo.genome.mutation

        # Store original mutation probabilities for potential unfreezing
        original_probs = {
            'node_add': mutation.node_add,
            'node_delete': mutation.node_delete,
            'conn_add': mutation.conn_add,
            'conn_delete': mutation.conn_delete,
        }

        # Zero out structural mutation probabilities
        # This disables node/connection add/delete in mutate_structure()
        mutation.node_add = 0.0
        mutation.node_delete = 0.0
        mutation.conn_add = 0.0
        mutation.conn_delete = 0.0

        # Re-compile JIT function to capture new zero probabilities
        # The mutation probabilities are Python if-checks at compile time
        self._compiled_tell = jax.jit(self.neat_algo.tell)

        # Store freeze flag and original probs in state
        if 'topology_frozen' in state:
            new_state = state.update(topology_frozen=True)
        else:
            new_state = state.register(
                topology_frozen=True,
                original_mutation_probs=original_probs
            )

        return new_state

    def unfreeze_topology(self, state: Any) -> Any:
        """Restore structural mutation probabilities.

        Undoes freeze_topology() by restoring original mutation rates.

        Args:
            state: Current algorithm state (must have been frozen)

        Returns:
            Modified state with structural mutations re-enabled
        """
        if not self.is_topology_frozen(state):
            return state  # Already unfrozen

        if 'original_mutation_probs' not in state:
            raise ValueError("Cannot unfreeze: original probabilities not stored")

        if not hasattr(self, 'neat_algo') or self.neat_algo is None:
            raise RuntimeError("Cannot unfreeze topology: neat_algo not initialized")

        # Restore original mutation probabilities
        original = state.original_mutation_probs
        mutation = self.neat_algo.genome.mutation

        mutation.node_add = original['node_add']
        mutation.node_delete = original['node_delete']
        mutation.conn_add = original['conn_add']
        mutation.conn_delete = original['conn_delete']

        # Re-compile JIT function with restored probabilities
        self._compiled_tell = jax.jit(self.neat_algo.tell)

        return state.update(topology_frozen=False)

    def is_topology_frozen(self, state: Any) -> bool:
        """Check if topology mutations are frozen."""
        # TensorNEAT State uses __contains__ for key lookup
        if 'topology_frozen' in state:
