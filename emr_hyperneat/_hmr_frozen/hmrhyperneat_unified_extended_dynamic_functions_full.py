"""HMR-HyperNEAT Unified Extended with Dynamic Functions and Neuromodulation.

This is the FULL implementation combining:
- hmrhyperneat_unified_extended.py: All multi-GPU, sparse H→H, multi-hop, caching features
- hmrhyperneat_dynamic_functions_aggregation.py: Per-node activation/aggregation functions
- hmrhyperneat_neuromodulation_functions.py: All 4 levels of neuromodulation
...
# Copied from hmrhyperneat_dynamic_functions_aggregation.py
...
def query_level_positions(
    ...
    for source_idx in range(num_sources):
        ...
        for chunk_start in range(0, pop_size, pop_chunk_size):
            ...
            # Query chunk for THIS level's positions only
    ...
    # Stack sources: (num_sources, pop_size, num_level_positions)
    # Transpose to: (pop_size, num_sources, num_level_positions)
...
def _forward_hybrid_sparse_hh(
    ...
    """
    # Step 1: Dense input→hidden
    h = jnp.tanh(inputs @ W1)  # (n_samples, num_positions)

    # Step 2: Sparse h→h iterations
    if activate_time > 1 and sparse_hh is not None and sparse_hh.num_valid.sum() > 0:
        ...
        def hh_step(h, _):
            ...
            source_vals = h[:, safe_from]  # (n_samples, max_sparse_conns)
            ...
            # Update with residual connection and activation
            return jnp.tanh(h + h_delta), None

        h, _ = lax.scan(hh_step, h, None, length=activate_time - 1)
...
def forward_hybrid_vmapped_dynamic(
    ...
    # Step 1: Dense input→hidden with activation
    pre_h = inputs @ W1  # (n_samples, num_positions)

    if act_indices is None:
        # Simple mode: use same activation for all nodes
        act_fn = hidden_act_fn if hidden_act_fn is not None else jnp.tanh
        h = act_fn(pre_h)
    elif palette is not None:
        # Palette mode: use grouped activation with custom palette
        h = grouped_activation_forward_with_palette(pre_h, act_indices, palette)
    else:
        # Standard grouped: use first N activations
        h = grouped_activation_forward(pre_h, act_indices, num_activations)
    ...
    if activate_time > 1:
        ...
        def compute_h_delta(h, contributions):
        ...
        if hh_activation_mode == 'every_iteration' and act_indices is not None:
            ...
            if palette is not None:
                def hh_step(h, _):
                    source_vals = h[:, safe_from]
                    contributions = source_vals * effective_weights
                    h_delta = compute_h_delta(h, contributions)
                    return grouped_activation_forward_with_palette(h + h_delta, act_indices, palette), None
            else:
                def hh_step(h, _):
                    source_vals = h[:, safe_from]
                    contributions = source_vals * effective_weights
                    h_delta = compute_h_delta(h, contributions)
                    return grouped_activation_forward(h + h_delta, act_indices, num_activations), None
        else:
...
def forward_hybrid_vmapped_neuromodulated(
    ...
    # Step 1: Dense input→hidden with NEUROMODULATED activation
    pre_h = inputs @ W1  # (n_samples, num_positions)

    # Apply gain modulation
    if modulation_mode == 'gain_bias_only' or modulation_mode == 'full':
        pre_h_modulated = effective_gains * pre_h
    else:
        pre_h_modulated = pre_h

    # Apply activation
    if act_indices is None:
        act_fn = hidden_act_fn if hidden_act_fn is not None else jnp.tanh
        h = act_fn(pre_h_modulated)
    elif palette is not None:
        h = grouped_activation_forward_with_palette(pre_h_modulated, act_indices, palette)
    else:
        h = grouped_activation_forward(pre_h_modulated, act_indices, num_activations)

    # Apply gating
    if modulation_mode == 'gating_only' or modulation_mode == 'full':
        h = h * gates

    # Step 2: Sparse h→h iterations (ALSO NEUROMODULATED per user requirement)
    if activate_time > 1:
        safe_from = jnp.clip(from_indices, 0, num_positions - 1)
        safe_to = jnp.clip(to_indices, 0, num_positions - 1)
        effective_weights = jnp.where(valid_mask, hh_weights, 0.0)
        ...
        if hh_activation_mode == 'every_iteration' and act_indices is not None:
                ...
                def hh_step(h, _):
                    ...
                    if modulation_mode == 'gain_bias_only' or modulation_mode == 'full':
                        h_updated = grouped_activation_forward_with_palette(
                            effective_gains * (h + h_delta), act_indices, palette
                        )
                    else:
                        h_updated = grouped_activation_forward_with_palette(
                            h + h_delta, act_indices, palette
                        )
                    # Apply gates
                    if modulation_mode == 'gating_only' or modulation_mode == 'full':
                ...
                def hh_step(h, _):
                    ...
                    if modulation_mode == 'gain_bias_only' or modulation_mode == 'full':
                        h_updated = grouped_activation_forward(
                            effective_gains * (h + h_delta), act_indices, num_activations
                        )
                    else:
                        h_updated = grouped_activation_forward(
                            h + h_delta, act_indices, num_activations
                        )
                    # Apply gates
                    if modulation_mode == 'gating_only' or modulation_mode == 'full':
                        h_updated = h_updated * gates
            ...
            def hh_step(h, _):
                ...
                if modulation_mode == 'gain_bias_only' or modulation_mode == 'full':
                    h_updated = jnp.tanh(effective_gains * (h + h_delta))
                else:
                    h_updated = jnp.tanh(h + h_delta)
                # Apply gates
                if modulation_mode == 'gating_only' or modulation_mode == 'full':
                    h_updated = h_updated * gates
                return h_updated, None
...
# ============================================================================
# Main Class: HMRHyperNEATUnifiedExtendedDynamicFunctions
# ============================================================================

class HMRHyperNEATUnifiedExtendedDynamicFunctions(HMRHyperNEATMultiGPU):
    """Unified Extended HMR-HyperNEAT with Dynamic Functions + Neuromodulation.
    ...
    Example with neuromodulation:
        algo = HMRHyperNEATUnifiedExtendedDynamicFunctions()
        algo.create_config({
            'algorithm_params': {
                'hmrhyperneat': {
                    'hmr_hyperneat': {
    ...
    def __init__(
        ...
        implementation: str = 'tensorneat-hmrhyperneat-unified-extended-dynamic-functions',
    ...
    def create_config(self, params: Dict[str, Any]) -> Any:
        ...
        if geo_seeding_enabled:
            ...
            # Inject this into the params BEFORE calling parent
            import copy
            params = copy.deepcopy(params)

            # Navigate to or create the cppn.genome.num_inputs path
            if 'algorithm_params' not in params:
                params['algorithm_params'] = {}
            if 'hmrhyperneat' not in params['algorithm_params']:
                params['algorithm_params']['hmrhyperneat'] = {}
            if 'cppn' not in params['algorithm_params']['hmrhyperneat']:
                params['algorithm_params']['hmrhyperneat']['cppn'] = {}
            if 'genome' not in params['algorithm_params']['hmrhyperneat']['cppn']:
                params['algorithm_params']['hmrhyperneat']['cppn']['genome'] = {}

            # Geometry seeding requires 7 CPPN inputs: [x1, y1, x2, y2, delta_x, delta_y, bias]
            params['algorithm_params']['hmrhyperneat']['cppn']['genome']['num_inputs'] = 7

            if self.verbose:
                print(f"[HMR-HyperNEAT] Geometry seeding: CPPN num_inputs set to 7 [x1, y1, x2, y2, delta_x, delta_y, bias]")
        ...
        if self.locality_radius is not None and self.verbose:
            print(f"[HMR-HyperNEAT] Locality radius enabled: {self.locality_radius}")
        ...
        if self.geometry_seeding_enabled and self.verbose:
            print(f"[HMR-HyperNEAT] Geometry seeding ENABLED (seed_weight={self.geometry_seeding_weight})")
            print(f"[HMR-HyperNEAT] CPPN inputs: [x1, y1, x2, y2, delta_x, delta_y, bias] (7 dimensions)")
    ...
    def _parse_dynamic_functions_config(self, hmr_config: Dict[str, Any]) -> None:
        ...
        Args:
            hmr_config: The 'hmr_hyperneat' section of config
        """
        ...
        self.num_cppn_outputs = dynamic_funcs.get('num_cppn_outputs', 1)
    ...
    def _parse_neuromodulation_config(self, hmr_config: Dict[str, Any]) -> None:
        ...
        if preset and preset in NEUROMODULATION_PRESETS:
            ...
            self.neuromod_config = NeuromodulationConfig(
                ...
                # Self-connection query for receptor densities
                use_self_connection_query=neuromod_config.get('use_self_connection_query', base.use_self_connection_query),
            )
        elif mode != 'disabled':
            # Build config from individual settings
            self.neuromod_config = NeuromodulationConfig(
    ...
    def run_generation_verbose(
        ...
        if self.dynamic_functions_mode == 'global':
            ...
            #
            # When num_cppn_outputs=1 (default, single-output CPPN):
            #   Use the weight values themselves to derive activation indices.
            #   This is similar to weight_interpretation but uses raw CPPN output directly.
            #
            # When num_cppn_outputs>1 (multi-output CPPN, future):
            #   Use dedicated CPPN output for activation (requires CPPN architecture change).
            #
            if self.num_cppn_outputs == 1:
                # Single-output CPPN: use W1 weights (the CPPN output) directly
                # W1 shape: (pop, num_inputs, num_positions)
                # Average across inputs to get per-position activation signal
                ...
                # Multi-output CPPN: would require querying CPPN with multiple outputs
                # This requires changes to the CPPN architecture and query functions
                raise NotImplementedError(
                    f"cppn_output mode with num_cppn_outputs={self.num_cppn_outputs} requires "
                    f"multi-output CPPN architecture support. Currently only num_cppn_outputs=1 "
                    f"is implemented, which derives activation from weight values."
                )

        # Step 4b: Compute H→H aggregation indices if dynamic mode enabled
        hh_agg_indices = None
        if self.hh_aggregation_mode == 'dynamic':
            # Use same interpretation logic as activation indices for H→H aggregation
            # This gives each hidden node a potentially different aggregation function
        ...
        if self.neuromod_config and self.neuromod_config.mode != 'disabled':
            ...
            if neuromod_mode == 'static_gating':
                ...
                if self.neuromod_config.use_self_connection_query:
                    # === PROPER METHOD: CPPN self-connection query (matches Implementation A) ===
                    # Query CPPN at (x,y,x,y,bias) for per-node properties, a true per-NODE
                    # property, not derived from per-connection weights.
                    all_positions_for_query = h_grid.all_positions
                    self_conn_outputs = batch_query_population_self_connections(
                        state, cppns_transformed, all_positions_for_query,
                        self._jitted_cppn_forward, self.num_cppn_outputs,
                        pop_chunk_size=self.population_chunk_size
                    )  # (pop_size, num_positions, num_cppn_outputs)
                    weight_values = self_conn_outputs[:, :, 0]  # (pop_size, total_positions)

                    receptor_densities = derive_receptor_from_weight(
                        weight=weight_values, method=receptor_derivation, num_nt_types=num_nt
                    )  # (pop, num_positions, num_nt)
                    receptor_densities = receptor_densities * 2.0 - 1.0
                    base_gains = jnp.abs(weight_values) * 0.5 + 0.5

                elif self.neuromod_config.receptor_from_weight:
                    # === LEGACY: Derive from W1 weights (kept for backward compatibility) ===
                    # W1 shape: (pop, num_inputs, num_positions)
                    # Uses first input's connection weight as proxy for per-node property.
                    weight_for_receptor = W1[:, 0, :]  # (pop, num_positions)
                    ...
                    raise NotImplementedError(
                        ...
                        "Use receptor_from_weight=True or use_self_connection_query=True."
                    )
        ...
        if sparse_hh is not None and self._forward_mode == ForwardPassMode.HYBRID_SPARSE_HH:
            ...
            if neuromod_data is not None:
                ...
                # Original dynamic evaluation without neuromodulation
                def eval_single_dense_dynamic(W1_i, W2_i, act_idx):
                    # inputs: (n_samples, num_inputs), W1_i: (num_inputs, total_positions)
                    pre_h = inputs_batch @ W1_i  # (n_samples, total_positions)

                    if act_idx is None:
                        # Global mode: use same activation for all nodes
                        h = hidden_act_fn(pre_h)
                    elif palette is not None:
                        # Palette mode
                        h = grouped_activation_forward_with_palette(pre_h, act_idx, palette)
                ...
                if eval_chunk_size and eval_chunk_size > 0 and pop_size > eval_chunk_size:
                    ...
                    # Full population vmap with dynamic functions
                    fitnesses = jax.vmap(eval_single_dense_dynamic)(W1, W2, act_indices)
            else:
        ...
        for level in range(num_levels):
            ...
            if level < num_levels - 1:
                ...
                level_variance = compute_single_level_variance(
                    cached_next_input_weights[:, 0, :],  # Use first input's weights
                    h_grid,
                    level,
                )
    ...
    def run_generation_multitask(
        ...
        hmrhyperneat_neuromodulation_functions.py.
    ...
    def _run_until_threshold_pmap(
        ...
        # Prepare HMR-HyperNEAT arrays (reusable across generations)
...
HMRHyperNEATUnifiedExtendedDynamicFunctionsFull = HMRHyperNEATUnifiedExtendedDynamicFunctions
...
__all__ = [
    ...
    'HMRHyperNEATUnifiedExtendedDynamicFunctions',
    'HMRHyperNEATUnifiedExtendedDynamicFunctionsFull',
