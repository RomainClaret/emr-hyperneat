"""EMR-HyperNEAT with Multi-GPU Support.

This module extends EMR-HyperNEAT with multi-GPU parallelization for accelerating
...
```python
from emr_hyperneat.emrhyperneat_pipeline_chunking_multi_gpus import (
    EMRHyperNEATMultiGPU,
    MultiGPUStrategy,
)

# RECOMMENDED for feedforward: Full pipeline on each GPU
algo = EMRHyperNEATMultiGPU(strategy=MultiGPUStrategy.FULL_PIPELINE_PARALLEL)

# RECOMMENDED for h→h modes: Only eval parallel, h→h cached
algo = EMRHyperNEATMultiGPU(strategy=MultiGPUStrategy.EVAL_ONLY_PARALLEL)

# FALLBACK: CPPN-only chunking (if FULL_PIPELINE_PARALLEL has issues)
algo = EMRHyperNEATMultiGPU(strategy=MultiGPUStrategy.CPPN_CHUNKED)

# Single GPU (for debugging)
algo = EMRHyperNEATMultiGPU(strategy=MultiGPUStrategy.SINGLE_GPU)
...
in emrhyperneat_chunking_multi_gpus_experimental.py for reference.
"""
...
# Use emr_hyperneat.sparse_forward_threshold in config instead.
...
# ============================================================================
# Integration - EMRHyperNEATChunking Class
# ============================================================================

class EMRHyperNEATChunking(BaseAlgorithm):
    """EMR-HyperNEAT with ADAPTIVE POPULATION CHUNKING.
    ...
    def __init__(self, name: str = 'emr-hyperneat',
                 implementation: str = 'tensorneat-emrhyperneat-adaptive'):
        ...
        # Execution mode parameters (set via create_config from emr_hyperneat section)
    ...
    def create_config(self, params: Dict[str, Any]) -> Any:
        ...
        algo_params = hierarchical_config.get('algorithm_params', {}).get('emrhyperneat', {})
        if not algo_params:
        ...
        if 'iteration_level' in hmr_config and hmr_config['iteration_level'] != 1:
            ...
            warnings.warn(
                ...
                "this implementation, which omits hidden→hidden "
        ...
        self.fitness_mode = hmr_config.get('fitness_mode', 'mse')

        # EMR-HyperNEAT execution mode parameters
        ...
        # Parse swarm mode configuration (multi-agent policy geometry)
        # When enabled, CPPN gets 2 extra inputs (agent_x, agent_y) prepended
        # to generate position-aware policies for each agent in a team
        swarm_section = algo_params.get('swarm', {})
        self.swarm_mode = swarm_section.get('enabled', False)
        self.swarm_num_agents = swarm_section.get('num_agents', 1)
        self.swarm_team_layout = swarm_section.get('team_layout', 'line')
        ...
        # With swarm mode: +2 for [agent_x, agent_y, ...] prepended
        default_num_inputs = 5
        if self.swarm_mode:
            default_num_inputs += 2  # agent_x, agent_y prepended
        flat_params = {
            'genome': {
                'num_inputs': cppn_genome_config.get('num_inputs', default_num_inputs),
        ...
        if cppn_activation:
            ...
            flat_params['genome']['activation'] = {
                'default': act_default, 'options': act_options, 'mutate_rate': act_mutate,
            }
            flat_params['activation_options'] = act_options
            flat_params['activation_default'] = act_default
            print(f"[CPPN] Custom activation: options={act_options}, "
                  f"default={act_default}, mutate_rate={act_mutate}")
        ...
        n_in = len(self.substrate_input_coords)
        n_out = len(self.substrate_output_coords)
        min_conns = n_in * n_out  # fully-connected initial layer
        dynamic_max_conns = max(2000, int(min_conns * 1.2))
        dynamic_max_nodes = max(500, n_in + n_out + 100)

        self.hyper_genome = RecurrentGenome(
            num_inputs=n_in,
            num_outputs=n_out,
            max_nodes=dynamic_max_nodes, max_conns=dynamic_max_conns,
    ...
    def _wrap_problem_for_pipeline(self, problem: Any) -> Any:
        ...
        # Determine CPPN input dimension:
        # Base: 5 [x1, y1, x2, y2, bias]
        # +2 with geometry seeding [delta_x, delta_y]
        # +2 with swarm mode [agent_x, agent_y] prepended
        cppn_input_dim = 5
        if getattr(self, 'geometry_seeding_enabled', False):
            cppn_input_dim += 2
        if getattr(self, 'swarm_mode', False):
            cppn_input_dim += 2
    ...
    def evaluate_on_data(
        ...
        # Detect multi-output CPPN (e.g., dynamic functions adds activation channel)
        num_cppn_outputs = getattr(self, 'num_cppn_outputs', 1)
        total_positions = h_grid.total_positions

        # Input → all positions: use explicit loop over source coords to avoid
        # numpy conversion errors inside vmap (batch_query_population_multi_source)
        input_weight_list = []
        for i in range(num_inputs):
            w = batch_query_population_positions(
                state, cppns_transformed, input_coords[i],
                all_positions, True, self._jitted_cppn_forward
            )
            # batch_query_population_positions returns (pop_size, total_positions * num_cppn_outputs)
            # due to .flatten() on multi-output CPPNs. Extract weight channel only.
            if num_cppn_outputs > 1:
                w = w.reshape(w.shape[0], total_positions, num_cppn_outputs)[:, :, 0]
            input_weight_list.append(w)
        input_all_weights = jnp.stack(input_weight_list, axis=1)

        # All → output: shape (pop_size, num_outputs, total_positions)
        output_weight_list = []
        for i in range(num_outputs):
            w = batch_query_population_positions(
                state, cppns_transformed, output_coords[i],
                all_positions, False, self._jitted_cppn_forward
            )
            if num_cppn_outputs > 1:
                w = w.reshape(w.shape[0], total_positions, num_cppn_outputs)[:, :, 0]
            output_weight_list.append(w)
        output_all_weights = jnp.stack(output_weight_list, axis=1)
...
def batch_query_population_positions_sharded(
    ...
    The caller (run_until_threshold in EMRHyperNEATMultiGPU) is
...
def _full_pipeline_single_gpu(
    ...
    """Run FULL EMR-HyperNEAT pipeline for a population slice on ONE GPU.
...
def _full_pipeline_single_gpu_with_hh(
    ...
    """Run FULL EMR-HyperNEAT pipeline with SPARSE H→H connections for a population slice.
...
def _full_pipeline_single_gpu_chunked(
    ...
    """Run FULL EMR-HyperNEAT pipeline with CHUNKING for large datasets.
...
class IslandModelRunner:
    ...
    Usage:
        runner = IslandModelRunner(
            algorithm_class=EMRHyperNEATChunking,
    ...
    def __init__(
        ...
        Args:
            ...
            max_depth: Max depth for EMR-HyperNEAT (used when config_path is None)
...
class IslandModelRunnerV2:
    ...
    def __init__(
        ...
        Args:
            ...
            max_depth: Max depth for EMR-HyperNEAT grid
        """
        self.algorithm_class = algorithm_class
        self.problem = problem
        self.config = config
...
class EMRHyperNEATMultiGPU(EMRHyperNEATChunking):
    """EMR-HyperNEAT with multi-GPU parallelization support.

    This class extends EMRHyperNEATChunking with
    ...
    def __init__(
        ...
        implementation: str = 'tensorneat-emrhyperneat-multi-gpu',
        ...
        """Initialize multi-GPU EMR-HyperNEAT.
    ...
    def _run_until_threshold_with_fixed_hh_single_gpu(
        ...
        from emr_hyperneat.emrhyperneat_unified_extended_dynamic_functions_full import (
